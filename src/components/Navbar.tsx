  ...
  return (
    <nav className="w-full h-full flex items-center font-bold overflow-hidden select-none z-40">
      <Link to="/algorithms" className="flex justify-center items-center">
        <svg
          className={`w-6 h-6 absolute left-0 ml-[24px] cursor-pointer ${location == "/algorithms"
              ? !darkMode
                ? "fill-[#000000]"
                : "fill-[#FFFFFF]"
      ...
      <div
        className={`w-full max-w-xl flex ${!isMobile ? "justify-between px-20" : "justify-center"
          } mx-auto text-center`}
      >
        {navlinks.map((link: navlink, index) => (
          <Link
            key={link.url}
            to={link.url}
            className={`${!isMobile ? "w-1/3" : "w-1/6"
              } mt-[2px] py-[16px] border-b-[2px] ${link.url === location
                ? !darkMode
      ...
      {!darkMode ? (
        <svg
          ...
          className={`w-6 h-6 absolute right-0 mr-[24px] ${!darkMode ? "hover:fill-[#000000]" : "hover:fill-[#FFFFFF]"
